"""Subpackage of tomtrix: plugins.

Contains all the first-party tomtrix plugins.
"""
...
from tomtrix.config import CONFIG
from tomtrix.plugin_models import FileType
from tomtrix.utils import cleanup, stamp

PLUGINS = CONFIG.plugins


class tomtrixMessage :
...
def load_plugins() -> Dict[str, tomtrixPlugin] :
    ...
    for item in PLUGINS :
        ...
        plugin_class_name = f"tomtrix{plugin_id.title()}"

        try :  # try to load first party plugin
            plugin_module = import_module( "tomtrix.plugins." + plugin_id )
        ...
        try :
            ...
            if not issubclass( plugin_class, tomtrixPlugin ) :
                logging.error(
                    f"Plugin class {plugin_class_name} does not inherit tomtrixPlugin"
                )
                continue
            plugin: tomtrixPlugin = plugin_class( item[1] )
...
async def apply_plugins(message: Message) -> tomtrixMessage :
    """Apply all loaded plugins to a message."""
    tm = tomtrixMessage( message )
