"""The module for running tomtrix in past mode.
...
from tomtrix import config
from tomtrix import storage as st
from tomtrix.config import CONFIG, get_SESSION, write_config
from tomtrix.plugins import apply_plugins
from tomtrix.utils import clean_session_files, send_message
...
async def forward_job() -> None :
    ...
    if CONFIG.login.user_type != 1 :
        logging.warning(
            "You cannot use bot account for tomtrix past mode. Telegram does not allow bots to access chat history."
        )
        return
    SESSION = get_SESSION()
