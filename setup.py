...
from tomtrix import storage as stg
from tomtrix.const import CONFIG_FILE_NAME
from tomtrix.plugin_models import PluginConfig
...
class LiveSettings( BaseModel ) :
    """Settings to configure how tomtrix operates in live mode."""
...
class BotMessages( BaseModel ) :
    ...
    bot_help: str = "For details visit github.com/rosteythetosser/tomtrix"


class Config( BaseModel ) :
    """The blueprint for tomtrix's whole config."""
...
def setup_mongo(client) :
    ...
    if not mycol.find_one( {"_id" : 0} ) :
        mycol.insert_one( {"_id" : 0, "author" : "tomtrix", "config" : Config().dict()} )
...
PASSWORD = os.getenv( "PASSWORD", "tomtrix" )
ADMINS = []

MONGO_CON_STR = os.getenv( "MONGO_CON_STR" )
MONGO_DB_NAME = os.getenv( "MONGO_DB_NAME", "tomtrix-config" )
MONGO_COL_NAME = os.getenv( "MONGO_COL_NAME", "tomtrix-instance-0" )
...
if PASSWORD == "tomtrix" :
    logging.warn(
        "You have not set a password to protect the web access to tomtrix.\nThe default password `tomtrix` is used."
    )
from_to = {}
